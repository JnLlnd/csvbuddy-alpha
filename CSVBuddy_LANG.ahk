...
global lLVeventsSortdescendingalphabetical := "Sort descending &alphabetical"
global lLVeventsSortdescendingnumericFloat := "Sort descending numeric (f&loat)"
global lLVeventsSortdescendingnumericInteger := "Sort &descending numeric (integer)"
...
global lTab4HelpExportExpress := "XML Export$Build a text file based on a row template where variable names are replaced with the content of each record in the collection.`n`nIn the ""Express template:"" zone, enter the template for each row of data in the collection. In this template, field names encapsulated by the character ¤ (ASCII code 164) are replaced by the matching data in each record. Also, ¤ROWNUMBER¤ is replaced by the current row number.`n`nAdditionaly, these special characters can be inserted in the template:`n`t``t`treplaced by Tab (HT)`n`t``n`treplaced by Linefeed (LF)`n`t``r`treplaced by Carriage return (CR)`n`t``f`treplaced by Form feed (FF)`n`nThe ""Express template:"" zone is initialized with all fields encapsulated by the character ¤ (ASCII code 164) and delimited with spaces.`n`nClick the ""Export"" button to export data and the ""Check"" button to see the result in the destination file."
...
global lTab5Abouttext3 := "by Jean Lalonde (<a href=""http://www.autohotkey.com/board/user/4880-jnllnd/"">JnLlnd</a> on AHK forum)"
